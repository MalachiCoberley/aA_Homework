function titleize(arr, cb) {
  let newArr = arr.map(el => `Mx. ${el} is a baby`);
  newArr.forEach(element => {
    console.log(element);
  });
}

titleize(["Jane", "Bob", "LeShawndra"]);
...
Elephant.prototype.play = function() {
  let randoTrick = this.tricks[Math.floor(Math.random()*this.tricks.length)];
  console.log(`${this.name} is ${randoTrick}`)
}
